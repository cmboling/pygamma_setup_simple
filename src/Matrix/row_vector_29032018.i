...
def __getitem__( self, sss ):        
	    ...
	    nnn = self.toNParray()
	    nnn = nnn[sss.start:sss.end:sss.step]

               
            array_length = len(nnn)
            new_row_vector = row_vector(array_length)
            for i in range(array_length):
                new_row_vector(i) = pygamma.complex(nnn[i].real,nnn[i].imag)
	    del nnn
...
#~ def __getitem__( self, sss ):        
        ...
        #~ if isinstance(sss,slice):
            ...
            #~ start = sss.
            #~ if sss.start == None and sss.stop == None and sss.step == None: # [::]
                #~ start = 0
                #~ stop  = self.__len__()
                #~ step  = 1
            #~ elif sss.start == None and sss.stop == None and sss.step > 0:   # [::>0]
                #~ start = 0
                #~ stop  = self.__len__()
                #~ step  = sss.step
            #~ elif sss.start == None and sss.stop == None and sss.step < 0:   # [::<0]
            ...
            #~ elif sss.start < 0 and sss.stop == None and sss.step == None:   # [<0::]
                #~ start = self.__len__()+sss.start
                #~ stop  = 0
                #~ step  = -1
            ...
            #~ elif sss.start == None and sss.stop >=0 and sss.step == None:   # [:>0:]
                #~ start = 0
                #~ stop  = sss.stop
                #~ step  = 1
            ...
            #~ for i in range( start, stop, step):
                ...
                #~ valr = self.getRe(i)
                #~ vali = self.getIm(i)
                #~ lll.append( complex( valr, vali ) )
                
            #~ array_length = len(lll)
            #~ new_row_vector = row_vector(array_length)
            #~ for i in range(array_length):

                #~ new_row_vector[i] = lll[i]
            #~ return( new_row_vector )
